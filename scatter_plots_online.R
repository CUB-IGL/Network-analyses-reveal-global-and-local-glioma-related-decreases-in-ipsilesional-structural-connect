...
#set parameters to prepare processing
#define path fo connectome matrices (in this case sd_stream results)
matrices_path = 'folder saving connectome matrices/'
matrices_prefix = 'S'
matrices_suffix = '.csv'

#read the nodes names and functional data 
patient_info = read_excel('connectome_table_online.xlsx') # read patients demographic data and clinical measures
name_nodes = read_excel('name_nodes.xlsx') # read the name of nodes
# set groups
patient_groups= c('all','precentral','postcentral','insular','frontal')
#set the alpha level for the tests
alpha = 0.05
...
for (subj in patient_info$Subj) {
  ...
  #to compute correllations of hemispheric edge differences, generate the difference matrices
...
####4
#####these loops can be run individually
for (subgroup in patient_groups){
  ######################all
  if (subgroup =='all'){
    data_loop<- patient_info %>% filter(all_patients==1)
    tfnbs_p =read.csv(paste0(matrices_path,'tfnbs_fwe.csv'),header = F,skip=1)
  }
  ######################precentral
  else if (subgroup =='precentral'){
    data_loop<- patient_info %>% filter(precentral==1)
    tfnbs_p =read.csv(paste0(matrices_path,'tfnbs_precentral_fwe.csv'),header = F,skip=1)
  }
  ######################postcentral
  else if (subgroup =='postcentral'){
    data_loop<- patient_info %>% filter(postcentral==1)
    tfnbs_p =read.csv(paste0(matrices_path,'tfnbs_postcentral_fwe.csv'),header = F,skip=1)
  }
  ######################insular
  else if (subgroup =='insular'){
    data_loop<- patient_info %>% filter(insular==1)
    tfnbs_p =read.csv(paste0(matrices_path,'tfnbs_insular_fwe.csv'),header = F,skip=1)
  }
  ######################frontal
  else if (subgroup =='frontal'){
    data_loop<- patient_info %>% filter(frontal==1)
    tfnbs_p =read.csv(paste0(matrices_path,'tfnbs_frontal_fwe.csv'),header = F,skip=1)
  }
  ...
  #go through all rows
  for (i in 1:40){
    #and then go through all columns
    for (e in 1:40){
      #if mask is 1 and thus p-value 0.05 or smaller
      if (tfnbs_p_mask[i,e]){
  ...
  #after calculating the correlations, correct with fdr
  ...
  #this shows only the adjusted p values in plots
  p_adj_index = 0
  for (i in 1:40){
    for (e in 1:40){
      if (tfnbs_p_mask[i,e]){
        ...
        #we need to run the correlation again because we only want to plot the edges with significant correlations
        cor_motorstatus = cor.test(diff_edge,data_loop$`Motor status`,method = "spearman",exact = F)
        cor_rmt = cor.test(diff_edge,data_loop$`RMT ratio`,method = "spearman",exact = F)
        cor_who = cor.test(diff_edge,data_loop$Histology,method = "spearman",exact = F)
        cor_nihss = cor.test(diff_edge,data_loop$`NIHSS(0-42)`,method = "spearman",exact = F)
        ...
        if (cor_motorstatus$p.value<0.05) {
          plot_motor <- ggplot(data_loop,aes(x = edge_temp,y = `Motor status`))+
            ...
            annotate("text", x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_motorstatus$estimate), "p = ",sprintf("%.4f", diff_motor_edge_p_adjust[p_adj_index])))
          
          ggsave(paste0(subgroup,'_motor_diff_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_motor,width = 9,height=9)
        }
        if (cor_who$p.value<0.05) {
          plot_who <- ggplot(data_loop,aes(x = edge_temp,y = Histology))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_who$estimate), "p = ",sprintf("%.4f", diff_who_edge_p_adjust[p_adj_index])))
          
          ggsave(paste0(subgroup,'_who_diff_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_who,width = 9,height=9)
        ...
        if (cor_nihss$p.value<0.05) {
          plot_nihss<- ggplot(data_loop,aes(x = edge_temp,y = `NIHSS(0-42)`,))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_nihss$estimate), "p = ",sprintf("%.4f", diff_nihss_edge_p_adjust[p_adj_index])))
          ggsave(paste0(subgroup,'_nihss_diff_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_nihss,width = 9,height=9)
        ...
        if (cor_rmt$p.value<0.05) {
          plot_rmt <- ggplot(data_loop,aes(x = edge_temp,y = `RMT ratio`))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text", x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_rmt$estimate), "p = ",sprintf("%.4f", diff_rmt_edge_p_adjust[p_adj_index])))
          ggsave(paste0(subgroup,'_rmt_diff_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_rmt,width = 9,height=9)
        }
      ...
      if (tfnbs_p_mask[i,e]){
        ...
        if (cor_motorstatus$p.value<0.05) {
          plot_motor <- ggplot(data_loop,aes(x = edge_temp,y = data_loop$`Motor status`))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_motorstatus$estimate), "p = ",sprintf("%.4f", patho_motor_edge_p_adjust[p_adj_index])))
          
          ggsave(paste0(subgroup,'_motor_patho_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_motor,width = 9,height=9)
        }
        ...
        if (cor_who$p.value<0.05) {
          plot_who <- ggplot(data_loop,aes(x = edge_temp,y = data_loop$Histology))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_who$estimate), "p = ",sprintf("%.4f", patho_who_edge_p_adjust[p_adj_index])))
          ggsave(paste0(subgroup,'_who_patho_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_who,width = 9,height=9)
        }
        if (cor_nihss$p.value<0.05) {
          plot_nihss<- ggplot(data_loop,aes(x = edge_temp,y = data_loop$`NIHSS(0-42)`,))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_nihss$estimate), "p = ",sprintf("%.4f", patho_nihss_edge_p_adjust[p_adj_index])))
          
          ggsave(paste0(subgroup,'_nihss_patho_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_nihss,width = 9,height=9)
        }
        if (cor_rmt$p.value<0.05) {
          plot_rmt <- ggplot(data_loop,aes(x = edge_temp,y = data_loop$`RMT ratio`))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_rmt$estimate), "p = ",sprintf("%.4f", patho_rmt_edge_p_adjust[p_adj_index])))
          
          ggsave(paste0(subgroup,'_rmt_patho_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_rmt,width = 9,height=9)
        }
      ...
      if (tfnbs_p_mask[i,e]){
        ...
        if (cor_motorstatus$p.value<0.05) {
          plot_motor <- ggplot(data_loop,aes(x = edge_temp,y = data_loop$`Motor status`))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_motorstatus$estimate), "p = ",sprintf("%.4f", healthy_motor_edge_p_adjust[p_adj_index])))
          
          ggsave(paste0(subgroup,'_motor_healthy_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_motor,width = 9,height=9)
        }
        ...
        if (cor_who$p.value<0.05) {
          plot_who <- ggplot(data_loop,aes(x = edge_temp,y = data_loop$Histology))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_who$estimate), "p = ",sprintf("%.4f", healthy_who_edge_p_adjust[p_adj_index])))
          
          ggsave(paste0(subgroup,'_who_healthy_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_who,width = 9,height=9)
        }
        if (cor_nihss$p.value<0.05) {
          plot_nihss<- ggplot(data_loop,aes(x = edge_temp,y = data_loop$`NIHSS(0-42)`,))+
            xlab('n streamlines')+ylab('NIHSS')+
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_nihss$estimate), "p = ",sprintf("%.4f", healthy_nihss_edge_p_adjust[p_adj_index])))
          
          ggsave(paste0(subgroup,'_nihss_healthy_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_nihss,width = 9,height=9)
        }
        if (cor_rmt$p.value<0.05) {
          plot_rmt <- ggplot(data_loop,aes(x = edge_temp,y = data_loop$`RMT ratio`))+
            ...
            geom_point(size=3) +geom_smooth(method='glm',alpha=I(0.2))+labs(title=paste0(name_nodes$name[i]," - ",name_nodes$name[e]))+
            theme_classic(base_size = 20)+
            annotate("text",x=x_position,y=5.5,size=10,label=paste("rs = ",sprintf("%.2f",cor_rmt$estimate), "p = ",sprintf("%.4f", healthy_rmt_edge_p_adjust[p_adj_index])))
          
          ggsave(paste0(subgroup,'_rmt_healthy_',name_nodes$name[i]," - ",name_nodes$name[e],".png"),plot = plot_rmt,width = 9,height=9)
        }
        healthy_edge=c()
      }
    }
